...
It this chapter we present the main result of this work: a hierarchy of stochastic, pure state trajectories in the spirit of the established density matrix hierarchy.
...
\end{equation}
% TODO Discussion what large enough k means
% TODO Its always saver to check convergence; usually much less orders required
...
\end{equation}
is called \quotes{terminator} of the hierarchy.
...
% FIXME Example
%As an example take the Fenna-Matthews-Olson complex further investigated in \autoref{sec:app.fmo}:
...
Since we assumed that the poles of the spectral density lie off the imaginary axis---or at least are distinct from the poles of the hyperbolic cotangent---\autoref{eq:num.alpha_thermal_re} for $t > 0$ reduces to a sum over all poles in the upper complex half-plane
\begin{equation}
...
% TODO CHECK HOW TO INCORPORATE COMPLEX PREFACTORS
...
\Autoref{fig:num.normcomp} shows the contribution of a few single realizations used on the left hand side of \autoref{fig:num.linvsnonlin}; clearly we obtain quite different results for both parameter sets.
